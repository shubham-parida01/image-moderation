...
async def moderate_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # result["image"] is a PIL.Image.Image object (either original or blurred)
        processed_image = result["image"]
        
        # Ensure we have a valid PIL Image
        if not isinstance(processed_image, Image.Image):
            logger.error(f"[API] Expected PIL.Image.Image, got {type(processed_image)}")
            raise HTTPException(status_code=500, detail="Invalid image object returned from pipeline")
        
        # Convert PIL Image to bytes (in-memory only, no disk save)
        img_byte_arr = io.BytesIO()
        processed_image.save(img_byte_arr, format='PNG')
        image_bytes = img_byte_arr.getvalue()
        
        logger.info(f"[API] Returning image in response with status: {result['status']}, size: {len(image_bytes)} bytes")
        
        # Return image directly in response - no file saving
        return Response(
            content=image_bytes,
        ...
        logger.exception(f"[API] Error processing image: {e}")
